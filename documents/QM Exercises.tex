    ...
    \subsection{已知两能量的本征态}
        ...
        \subsection{表象问题}
            ...
            \begin{formal}
                ...
                $$ \psi(p) = \braket{p}{\psi} = \int dx' \braket{p}{x'} \braket{x'}{\psi}  = \intff \psi(x) e^{\frac{-ipx}{i\hbar}} dx $$
                $$ \psi(x) = \braket{x}{\psi} = \int dp' \braket{x}{p'} \braket{p'}{\psi} = \psi(x) = \intff \psi(p) e^{\frac{ipx}{i\hbar}}  dp $$
            \end{formal}
        ...
        \subsection{动量波函数}
            ...
            使用表格分部积分法即可,概率密度是实数,所以只取欧拉公式的实数部分
            $$ Re[e^{i x}] = Re[\cos{x}+i\sin{x}] = \cos{x} $$
        ...
        \subsection{阶跃势的透射与反射问题}
            ...
            但是透射参数$T \neq \abs{F}^{2}$(解的形式不一样),所以需要先计算反射系数$R$,参数带虚数指标$i$时,要取复共轭来计算  
