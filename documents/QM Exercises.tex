    ...
    \section{近似方法}
        ...
        \begin{formal}
            ...
            \begin{enumerate}
                ...
                常见试探波函数-高斯函数$ \psi(x) = A e^{-b x^{2}} \quad A =(\frac{2b}{\pi})^{\frac{1}{4}} $
                ...
                \item 精细结构:相对论修正和自旋-轨道耦合
                    ...
                    \begin{itemize}
                        \item 相对论修正($m$为静质量)
                        $$ T = \gamma m c^{2} - m c^{2} \quad p = \gamma m v \lra (T + mc^{2})^{2} = p^{2}c^{2} + m^{2}c^{4} $$
                        $\frac{p}{mc}$作为小量进行泰勒展开,取第一级作为哈密顿量的相对论修正(作为微扰)
    ...
    \section{自旋}
        ...
        \begin{enumerate}
            ...
            \item 在$\vu*{S}_{z}$表象下
                  ...
                  \comm{\sigma_{i}}{\sigma_{j}} = 2i \sigma_{k} \varepsilon_{ijk}
                  $$

                  $$
                  \sigma_{x}\sigma_{y} = - \sigma_{y}\sigma_{x} = i \sigma_{z} \quad 
                  \sigma_{y}\sigma_{z} = - \sigma_{z}\sigma_{y} = i \sigma_{x} \quad 
                  \sigma_{z}\sigma_{x} = - \sigma_{x}\sigma_{z} = i \sigma_{y}
                  $$
