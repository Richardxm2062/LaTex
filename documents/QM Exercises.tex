...
\begin{document}
    ...
    \subsection{已知两能量的本征态}
        ...
        \subsection{海森堡绘景2}
            \begin{thm}\label{thm:1.3}
                ...
                维里定理:动能平均值是势能的平均值的$\frac{v}{2}$倍,其中$v$表示势能函数是关于$x$的$v$次方程
    ...
    \section{力学量算符}
        \begin{formal}
            ...
            $$ \comm{x_{i}}{\vu*{p}_{j}} = i \hbar \delta_{ij} \quad \comm{\vu*{L}_{i}}{\vu*{L}_{j}} = i\hbar \vu*{L}_{k} \varepsilon_{ijk}  
            ...
            如果一个算符写在了幂指数上例如$e^{\lambda \vu*{A}}$,那么它和别的算符的对易关系可以直接看作算符$\vu*{A}$来使用,比如
            ...
            $$ \triangle{A} \triangle{B} \geq \frac{1}{2} \abs{\overline{\comm{A}{B}}} $$
            $$ \triangle{A} = \sqrt{<A^{2}> - <A>^{2}} $$
            $$ \dv{\vu*{F}(t)}{x} = \pdv{\vu*{F}(t)}{t} + \frac{1}{i \hbar } [\vu*{F}(t),\vu*{H}] $$

            力学量不含时间且与$\vu*{H}$对易时,此力学量为守恒量
            ...
            假设粒子的束缚定态能量为$E_{n}$,相应的归一化波函数为$\psi_{n}$,$\nabla$为哈密顿算符$\vu*{H}$中的任一参数,便有
            $$ \pdv{E_{n}}{\lambda} = \overline{\qty(\pdv{\vu*{H}}{\lambda})_{n}} $$

            一维度谐振子的升降算符
            $$ \ua = \frac{1}{ \sqrt{2m \hbar w}} ( -i\vu*{p} + mwx ) \quad \da = \frac{1}{ \sqrt{2m \hbar w}} ( +i\vu*{p} + mwx ) $$
            $$ \comm{a_{-}}{a_{+}} = 1 \quad \ua \ket{n} = \sqrt{n+1} \ket{n+1} \quad \da \ket{n} = \sqrt{n} \ket{n-1} $$
            $\da$ 与$\ua$可以构成厄米算符$\vu*{N}=\da \ua$,且$\vu*{N}$的本征值为$n=0,1,2,3\cdots$,所以哈密顿量又可以写为
            $$ \vu*{H} = \hbar w (\da \ua - \frac{1}{2}) \llra \vu*{H} = (\vu*{N} + \frac{1}{2})\hbar w $$
        \end{formal}
        ...
        \subsection{角动量算符的本征值问题}
            ...
            \begin{formal}
                ...
                $$ \vu*{L}_{\pm} Y_{lm}(\theta,\varphi) = \sqrt{l(l+1) - m(m \pm 1)}  \hbar Y_{lm \pm 1}(\theta,\varphi) $$
        ...
        \subsection{谐振子的均值问题}
            ...
            \begin{formal}
                ...
                \begin{itemize}
                    ...
                    \item   递推方式 
                    $$  x \psi_{n} = \frac{1}{\alpha}(\sqrt{\frac{n}{2}}\psi_{n-1} + \sqrt{\frac{n+1}{2}} \psi_{n+1})  \quad \dv{\psi_{n}}{x} = \alpha (\sqrt{\frac{n}{2}}\psi_{n-1} - \sqrt{\frac{n+1}{2}} \psi_{n+1}) $$
        ...
        \subsection{动量表象下的定态能量和波函数}
            ...
            \begin{formal}
                ...
                \begin{itemize}
                    ...
                    \begin{proof}
                        \pfindent
                        $$ x = b\tan{\theta} \quad dx = b \frac{1}{\cos{\theta}^{2}} d\theta \quad 1+\tan{\theta}^{2} = \frac{1}{\cos{\theta}^{2}} $$
